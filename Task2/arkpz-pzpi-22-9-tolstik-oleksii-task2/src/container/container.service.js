export class ContainerService {
    ...
    async getContainerDetails(db, id) {
        try {
            ...
            const response = {
                ...
                patient: container.Patient ? `${container.Patient.last_name} ${container.Patient.first_name} ${container.Patient.patronymic}` : 'Немає пацієнта',
    ...
    async addPatientToContainer(db, id, patient_id) {
        try {
            ...
            const newPatient = await db.patient.findUnique({
                where: {
                    id_patient: Number(patient_id),
                },
                select: {
